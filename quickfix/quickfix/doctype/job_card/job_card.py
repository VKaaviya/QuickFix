...
class JobCard(Document):
    ...
    def before_submit(self):
        if self.status not in ["Ready for Delivery", "Delivered"]:
            frappe.throw(
                frappe._(
                    "Job card can only be submitted when status is 'Ready for Delivery' or 'Delivered'."
                )
            )
    ...
    def on_cancel(self):
        ...
        doc_name = frappe.db.get_value(
            "Service Invoice", {"job_card": self.name, "docstatus": 1}, "name"
        )
        if doc_name:
    ...
    def deduct_part_stock(self):
        if self.parts_used:
            for part in self.parts_used:
                stock_quantity = flt(
                    frappe.db.get_value("Spare part", part.part, "stock_qty")
                )
        ...
        if self.parts_used:
            for row in self.parts_used:
                if flt(row.quandity) <= 0:
                    frappe.throw(
                        f"Quantity must be greater than zero for part {row.part}"
                    )
    ...
    def validate_assigned_technician(self):
        if (
            self.status in ["In Repair", "Ready for Delivery", "Delivered"]
