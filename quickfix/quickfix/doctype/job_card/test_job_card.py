...
def make_job_card(**overrides):
    ...
    device_type = overrides.pop("device_type", None) or make_device_type()

    technician = overrides.pop("technician", None) or make_technician()
...
class IntegrationTestJobCard(IntegrationTestCase):
    ...
    def test_final_amount(self):
        ...
        spare_part_1 = make_spare_part(
            unit_cost=100.0, selling_price=150.0, stock_qty=20
        )

        spare_part_2 = make_spare_part(
            unit_cost=200.0, selling_price=300.0, stock_qty=20
        )
    ...
    def test_child_table_computation(self):
        ...
        spare_part_1 = make_spare_part(
            unit_cost=50.0, selling_price=100.0, stock_qty=20
        )
    ...
    def test_parts_availability_check(self):
        ...
        job_card.append("parts_used", {"part": spare_part.name, "quandity": 1})
        job_card.save()
        with self.assertRaises(frappe.ValidationError):
            job_card.submit()
        job_card.reload()
    ...
    def test_deduct_stock(self):
        ...
        self.assertEqual(
            frappe.db.get_value("Spare part", spare_part_1.name, "stock_qty"), 17
        )
        self.assertEqual(
            frappe.db.get_value("Spare part", spare_part_2.name, "stock_qty"), 15
        )

        job_card.cancel()
        self.assertEqual(
            frappe.db.get_value("Spare part", spare_part_1.name, "stock_qty"), 20
        )
        self.assertEqual(
            frappe.db.get_value("Spare part", spare_part_2.name, "stock_qty"), 20
        )
    ...
    def test_service_invoice_creation(self):
        ...
        job_card.append("parts_used", {"part": self.spare_part.name, "quandity": 2})
    ...
    def test_invoice_cancellation(self):
        ...
        invoice_name = frappe.db.get_value(
            "Service Invoice", {"job_card": job_card.name}, "name"
        )
        ...
        invoice_docstatus = frappe.db.get_value(
            "Service Invoice", invoice_name, "docstatus"
        )
    ...
    def test_duplicate_invoice(self):
        ...
        invoices = frappe.get_all(
            "Service Invoice", filters={"job_card": job_card.name}
        )
    ...
    def test_same_part_used_twice(self):
        ...
        spare_part = make_spare_part(unit_cost=100.0, selling_price=150.0, stock_qty=20)

        job_card = make_job_card()

        job_card.append(
            "parts_used", {"part": spare_part.name, "quandity": 2, "unit_price": 150.0}
        )

        job_card.append(
            "parts_used", {"part": spare_part.name, "quandity": 3, "unit_price": 150.0}
        )

        job_card.save()

        self.assertEqual(job_card.parts_used[0].total_price, 300.0)

        self.assertEqual(job_card.parts_used[1].total_price, 450.0)
    ...
    def test_zero_quantity_row(self):
        ...
        job_card.append(
            "parts_used", {"part": spare_part.name, "quandity": 0, "unit_price": 100.0}
        )
